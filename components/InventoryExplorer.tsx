...
const products = [
  {
    ...
    description:
      'Ventilated, slip-resistant floors, and ample interior space. Ideal for job sites.',
  ...
  },
  {
    ...
    description:
      'Climate-controlled trailer with flushing toilets, running water, and elegant finishes.',
    image: '/images/deluxe-unit-sink.png',
...
const InventoryExplorer: React.FC = () => {
  ...
  const toggleQuoteItem = (id: number) => {
    setQuoteItems((prev) => (prev.includes(id) ? prev.filter((i) => i !== id) : [...prev, id]));
  };
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          <p className="text-lg text-zinc-600">
            Browse our complete catalog of sanitation solutions, restroom trailers, and site
            equipment.
          </p>
        </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <p className="text-zinc-600">
                <span className="font-bold text-zinc-900">{filteredProducts.length}</span> products
                found
              </p>
              {hasActiveFilters && (
                <button
          ...
          <AnimatePresence mode="popLayout">
            {filteredProducts.map((product) => (
              <motion.div
                ...
                className={`group bg-white rounded-3xl overflow-hidden border border-zinc-200 hover:shadow-2xl hover:-translate-y-1 transition-all duration-500 ${
              ...
              >
                ...
                <div
                  className={`relative overflow-hidden bg-zinc-100 ${
                    viewMode === 'list'
                      ? 'md:w-72 aspect-[16/10] md:aspect-auto md:h-auto'
                      : 'aspect-[16/10]'
                  }`}
                >
                ...
                <div className={`p-6 flex flex-col ${viewMode === 'list' ? 'flex-1' : ''}`}>
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    <div className="flex items-center gap-1">
                      {[...Array(5)].map((_, i) => (
                        <svg
                          key={i}
                          className={`w-4 h-4 ${i < Math.floor(product.rating) ? 'text-amber-400' : 'text-zinc-200'}`}
                          fill="currentColor"
                          viewBox="0 0 20 20"
                        >
                  ...
                  {/* Features */}
                  <div
                    className={`flex flex-wrap gap-2 mb-4 ${viewMode === 'list' ? 'md:mb-0' : ''}`}
                  >
                    {product.features.slice(0, 3).map((feature, idx) => (
                      <span
                        key={idx}
                        className="px-3 py-1 bg-zinc-100 text-zinc-600 text-xs font-bold rounded-full"
                      >
                        {feature}
                      </span>
                    ))}
                  ...
                  {/* Price & CTA */}
                  <div
                    className={`flex items-center justify-between pt-4 border-t border-zinc-100 ${viewMode === 'list' ? 'md:pt-0 md:border-0 md:ml-auto md:gap-6' : ''}`}
                  >
                    <div>
                    ...
                    <button
                      ...
                      className={`px-5 py-3 rounded-xl font-bold text-sm transition-all ${
                        quoteItems.includes(product.id)
