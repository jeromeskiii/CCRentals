...
const QuoteCalculator: React.FC<QuoteCalculatorProps> = ({ onRequestQuote }) => {
  ...
  const calculateQuote = () => {
    ...
    const durationCharge = tier.perDayRate * Math.max(0, duration - 1);
    const unitCharge = tier.perUnitRate * Math.max(0, units - 1);
    const deliveryFee = units >= 5 ? 0 : 75;
        ...
        <div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 text-center">
              <div className="text-3xl font-bold text-primary">{units}</div>
              <div className="text-xs text-muted-foreground">
                {units === 1 ? 'unit' : 'units'}
              </div>
            </div>
