...
export const Card = ({ children, className = "", noPadding = false }: { children: React.ReactNode; className?: string; noPadding?: boolean }) => (
  <div className={`border border-stone-400 bg-stone-50/50 backdrop-blur-sm relative overflow-hidden group ${className}`}>
...
export const Badge = ({ status, label }: { status: string; label?: string }) => {
  ...
  return (
    <span className={`px-2 py-0.5 text-[10px] uppercase tracking-widest font-mono border ${styles[status] || styles.locked}`}>
      {label || status}
    </span>
  );
};

export const DataPoint = ({ label, value, mono = false }: { label: string; value: string; mono?: boolean }) => (
  <div className="flex flex-col">
    <span className="text-[10px] uppercase text-stone-500 tracking-wider mb-1">{label}</span>
    <span className={`text-sm ${mono ? 'font-mono' : 'font-medium'} border-l-2 border-stone-300 pl-3 -ml-3 hover:border-orange-500 hover:pl-3 transition-all duration-300`}>
      {value}
    </span>
  </div>
);
