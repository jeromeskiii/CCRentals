...
export function showSaveProgress(sessionId) {
  process.stderr.write(`\r${colorize("⏺", COLORS.green)} Saving as session: ${colorize(sessionId, COLORS.cyan)}...`);
}

export function showSaveComplete(session) {
  process.stderr.write(`\r${colorize("✓", COLORS.green)} Session saved: ${colorize(session.sessionId, COLORS.cyan)}\n`);

  if (session.repoAttached) {
    process.stderr.write(`  ${colorize("✓", COLORS.green)} Repo attached: ${colorize(session.repoFileCount, COLORS.yellow)} files\n`);
  }

  if (session.indexTier === 2 && session.lspCapabilities) {
    const langs = session.lspCapabilities.availableLanguages.join(", ");
    process.stderr.write(`  ${colorize("✓", COLORS.green)} Tier 2 enabled: ${colorize(langs, COLORS.yellow)}\n`);
  }
}
