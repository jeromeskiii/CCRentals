...
function ensureFetchAvailable() {
  if (typeof fetch !== "function") {
    throw new Error(
      "Global fetch is not available. Use Node 18+ or higher to run blink."
    );
  }
}
...
function checkRateLimit() {
  ...
  if (!status.allowed) {
    throw new Error(
      `Rate limit exceeded. ${status.message}`
    );
  }
...
function createOpenAIModelFromEnv() {
  ...
  if (!apiKey) {
    throw new Error(
      "Missing BLINK_API_KEY or OPENAI_API_KEY for OpenAI provider."
    );
  }

  const modelId = process.env.BLINK_MODEL || "gpt-4.1-mini";
  const baseUrl =
    process.env.BLINK_API_BASE || "https://api.openai.com/v1/chat/completions";
  ...
  return {
    ...
    async *complete({ prompt }) {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `OpenAI request failed with status ${response.status}: ${text}`
        );
      }
...
function createAnthropicModelFromEnv() {
  ...
  if (!apiKey) {
    throw new Error(
      "Missing BLINK_API_KEY or ANTHROPIC_API_KEY for Anthropic provider."
    );
  }

  const modelId = process.env.BLINK_MODEL || "claude-3-5-sonnet-20241022";
  const baseUrl =
    process.env.BLINK_API_BASE || "https://api.anthropic.com/v1/messages";
  ...
  return {
    ...
    async *complete({ prompt }) {
      ...
      if (!response.ok) {
        const text = await response.text();
        throw new Error(
          `Anthropic request failed with status ${response.status}: ${text}`
        );
      }
...
function createMinimaxModelFromEnv() {
  ...
  if (!apiKey) {
    throw new Error(
      "Missing BLINK_API_KEY or MINIMAX_API_KEY for MiniMax provider."
    );
  }

  const modelId = process.env.BLINK_MODEL || "MiniMax-M2.1";
  const baseUrl =
    process.env.BLINK_API_BASE || "https://api.minimax.io/anthropic/v1/messages";
  ...
  return {
    ...
    async *complete({ prompt }) {
      ...
      if (!response.ok) {
        const text = await response.text();
        throw new Error(
          `MiniMax request failed with status ${response.status}: ${text}`
        );
      }
